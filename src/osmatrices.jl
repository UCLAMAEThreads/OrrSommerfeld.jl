...
"""
    os_matrices(α,β[;baseflow=:tanhmixing, N=200]) -> OSMatrix

Returns the Rayleigh equation matrices for a given base flow (between walls at y = +1/-1), for
streamwise wavenumber `α` and spanwise wavenumber `β`.
The default base flow is a tanh mixing layer, 0.5*(1+tanh(7y)), but this can be changed to `:couette` for
Couette flow. Optional argument `N` specifies the number of Chebyshev modes to use.
It defaults to 200.
"""
function os_matrices(α::Real,β::Real; N::Integer=200,baseflow=:tanhmixing)
    C = Cheb(N)
...
function _os_eigen_raw(d::OSMatrix{N},minval,maxval,normalize,categorize,::Val{:os}) where N
    ...
    return OSEigen(collect(1:length(λu)),Integer[],λu,Xu)
end
...
function _os_eigen_raw(d::OSMatrix{N},minval,maxval,normalize,categorize,::Val{:sq}) where N
    ...
    λu,Xu = _limit_eigenvalues(λ,X,minval,maxval)
    return OSEigen(Integer[],collect(1:length(λu)),λu,Xu)
end
